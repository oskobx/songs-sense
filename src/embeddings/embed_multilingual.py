"""Substep: Embed all passages with bge-m3 (dense mode) into embedding_multi column.

Usage:
    python -m src.embeddings.embed_multilingual              # test 100, prompt to continue
    python -m src.embeddings.embed_multilingual --skip-test  # full run, no prompt
    python -m src.embeddings.embed_multilingual --index-only # skip embedding, build HNSW index

Environment:
    PYTORCH_ENABLE_MPS_FALLBACK=1 recommended on Apple Silicon.
"""
...
def load_model(device: str):
    ...
    print(f"Loading {MODEL_NAME} on {device.upper()} (~2.3 GB first download)...")
...
def fetch_null_batch(conn: psycopg.Connection, limit: int) -> list[tuple[int, str]]:
    return conn.execute(
        "SELECT id, passage_text FROM passages WHERE embedding_multi IS NULL LIMIT %s",
        (limit,),
    ).fetchall()


def write_embeddings(conn: psycopg.Connection, ids: list[int], vecs: np.ndarray) -> None:
    with conn.pipeline():
        for pid, vec in zip(ids, vecs):
            conn.execute(
                "UPDATE passages SET embedding_multi = %s WHERE id = %s",
                (vec, pid),
            )
    conn.commit()
...
def count_null(conn: psycopg.Connection) -> int:
    return conn.execute(
        "SELECT COUNT(*) FROM passages WHERE embedding_multi IS NULL"
    ).fetchone()[0]
...
def run_test(model, conn: psycopg.Connection) -> float:
    print("\n--- Test run: first 100 passages ---")
    rows = conn.execute(
        "SELECT id, passage_text FROM passages WHERE embedding_multi IS NULL LIMIT 100"
    ).fetchall()
    ...
    print(f"  Sample emb[:5]    : {vecs[0][:5].tolist()}")
...
def estimate_runtime(throughput: float, null_count: int) -> None:
    ...
    print(f"  Estimated runtime       : {remaining_sec/60:.1f} min")
...
def embed_all_passages(model, conn: psycopg.Connection) -> None:
    ...
    sample_rows = conn.execute(
        "SELECT id, passage_text, embedding_multi FROM passages "
        "WHERE embedding_multi IS NOT NULL ORDER BY random() LIMIT 5"
    ).fetchall()
    norms = [np.linalg.norm(np.array(r[2])) for r in sample_rows]
    print(f"  Avg norm (sample) : {np.mean(norms):.6f}  (should be ~1.0)")
    for row in sample_rows:
        vec = np.array(row[2])
        print(f"    id={row[0]}  embedding_multi[:5]={vec[:5].tolist()}")
...
def create_hnsw_index(conn: psycopg.Connection) -> None:
    print("\n--- Building HNSW index on embedding_multi ---")
    t0 = time.perf_counter()
    conn.execute("SET statement_timeout = 0")
    conn.execute(
        f"""
        CREATE INDEX IF NOT EXISTS passages_embedding_multi_hnsw_idx
        ON passages
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Embed passages with bge-m3 (multilingual)")
